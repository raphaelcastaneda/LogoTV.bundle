...
MRSS_URL = "http://www.logotv.com/player/includes/rss.jhtml?uri=mgid:uma:videolist:logotv.com:%s"
...
def MoreVideos(title, url, show_type):
  ...
  for video in data.xpath('//div[@id="carousel-%s"]/div/a' %show_type):
    ...
    if vid_url.endswith('series.jhtml'):
      oc.add(DirectoryObject(key=Callback(ShowVideos, title=title, url=vid_url), title=title, thumb=Resource.ContentsOfURLWithFallback(url=thumb, fallback=ICON)))
    elif vid_url.endswith('playlist.jhtml'):
      ...
      if 'movies' in show_type:
        ...
        try:
          episode = int(RE_EPISODE.search(vid_url).group(1))
        except:
          episode = 0
        try:
          season = int(RE_SEASON.search(vid_url).group(1))
        except:
          season = 1
      ...
      if '?id' in vid_url:
        ...
        rss_url = MRSS_URL %vid_id
        # send to videopage function
        oc.add(DirectoryObject(key=Callback(RSSPage, title=title, url=rss_url, thumb=thumb), title=title, thumb=Resource.ContentsOfURLWithFallback(url=thumb, fallback=ICON)))
      else:
        pass
...
# Full episodes and movies have a value for @mainuri
...
def ShowVideos(title, url):
  ...
  for video in data.xpath('//ol/li[@itemtype="http://schema.org/VideoObject"]'):
    ...
    if content != 'Full Episode':
      ...
      # Most of Logos other videos are playlist with as many as 30 videos, so check for it and produce full list here
        vid_id = vid_url.split('#id=')[1]
        rss_url = MRSS_URL %vid_id
        oc.add(DirectoryObject(key=Callback(RSSPage, title=title, url=rss_url, thumb=thumb), title=title, thumb=thumb))
      else:
  ...
  if len(oc) < 1:
    ...
    return oc
####################################################################################################
# This produces videos video playlist broken down with the RSS feed
# The url for thumbs produced from these RSS feed pages give errors and is not resolved with fallback, so just using prior thumb sent to function
@route(PREFIX + '/rsspage')
def RSSPage(url, title, thumb):
  oc = ObjectContainer(title2=title)
  data = XML.ElementFromURL(url)
  for item in data.xpath('//item'):
    link = item.xpath('./link//text()')[0]
    #image = item.xpath('./image//@url')[0]
    video_title = item.xpath('./title//text()')[0]
    ...
    oc.add(VideoClipObject(url=link, title=video_title, originally_available_at=date, thumb=thumb))
